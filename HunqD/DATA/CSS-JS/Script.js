  ...
  if (result && result.pass === passwordInput) { // Kiểm tra mật khẩu
    ...
    resultDiv.innerHTML = `
        ...
        <div class="Head">
            ...
            <div class="Avatar">
                <img src="https://graph.facebook.com/61551995024526/picture?width=9999&access_token=6628568379|c1e620fa708a1d5696fb991c1bde5662" alt="" srcset="">
            </div>
        </div>
        <div class="Time">
...
function searchByNameOrID(query) {
  // Kiểm tra nếu query chứa URL đầy đủ
  const facebookURL = extractFacebookProfileURL(query);
  if (facebookURL) {
    // Nếu có URL, thực hiện tìm kiếm bằng URL
    ...
    // Nếu không phải URL đầy đủ, tiếp tục tìm kiếm bằng tên hoặc ID
    for (const data of jsonData[0]) {
