...
async function fetchWeather(latitude, longitude) {
    ...
    document.getElementById('location').innerText = `Vị trí: ${data.location.name}, ${data.location.region}, ${data.location.country}`;
    document.getElementById('temperature').innerText = `Nhiệt độ: ${data.current.temp_c}°C`;
    document.getElementById('rain_chance').innerText = `Khả năng mưa: ${data.current.precip_mm} mm`;
    document.getElementById('uv_index').innerText = `Mức tia UV: ${data.current.uv}`;

    // Chuyển đổi gió hướng sang định dạng thuần Việt
    const windDir = data.current.wind_dir;
    const windDirection = windDirectionMap[windDir] || windDir;
    document.getElementById('wind_direction').innerText = `Gió hướng: ${windDirection} với tốc độ ${data.current.wind_kph} km/h`;
